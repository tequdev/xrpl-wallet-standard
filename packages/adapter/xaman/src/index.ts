import { type XRPLBaseWallet, XRPLWalletAccount, hexToBytes, bytesToHex } from '@xrpl-wallet-adapter/base'
import {
...
export class XamanWallet implements XRPLBaseWallet {
  ...
  #signTransaction: XRPLSignTransactionMethod = async ({ transaction, account, network }) => {
    const { payload } = await this.#sdk.payload.createAndSubscribe(
      { txblob: bytesToHex(transaction), options: { submit: false } },
    )
    // const hash = payload.response.txid!
    const tx_blob = payload.response.hex!
    return {
      signed_tx: Uint8Array.from(hexToBytes(tx_blob)),
      // TODO
  ...
  #signAndSubmitTransaction: XRPLSignAndSubmitTransactionMethod = async ({ transaction, account, chain }) => {
    ...
    return {
      tx_blob: Uint8Array.from(hexToBytes(tx_blob)),
      tx_hash: Uint8Array.from(hexToBytes(hash)),
    }
  }
